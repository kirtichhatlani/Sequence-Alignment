...
for i in range(1,m+1):
    for j in range(1, n+1):
        if seq1_fasta[i-1] == seq2_fasta[j-1]:
            matrix[i][j] = max(matrix[i][j-1]+gap_penalty, matrix[i-1][j]+gap_penalty, matrix[i-1][j-1]+match)
        else:
            matrix[i][j] = max(matrix[i][j-1]+gap_penalty, matrix[i-1][j]+gap_penalty, matrix[i-1][j-1]+mismatch)
...
while (m>0 or n>0):
    ...
    #If it is a match
    if seq1_fasta[m-1] == seq2_fasta[n-1]:
        seq1 += seq1_fasta[m-1]
        seq2 += seq2_fasta[n-1]
        m -= 1
        n -= 1
    ...
    #If it is a mismatch:
    elif seq1_fasta[m-1] != seq2_fasta[n-1]:
        list = [matrix[m-1][n-1], matrix[m-1][n], matrix[m][n-1]]        
